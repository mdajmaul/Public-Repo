...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        // GBAN check removed: no longer trusts a third-party remote ban-list to shut down this bot.
                        let dataGban = {};

                        // Remote notification removed: no longer fetches/displays messages from a third-party source.
                        let notification = "";
                        ...
                        // Tertiary anti-logout: Check connection every 10 minutes (safe, no getUserInfo spam)
                        setInterval(async () => {
                                try {
                                        // Use getAppState instead of getUserInfo to avoid Facebook rate limit/block
                                        const state = api.getAppState();
                                        if (!state || state.length === 0) {
                                                throw new Error("AppState empty");
                                        }
                                        console.log(`[ANTI-LOGOUT] ✓ Connection OK at ${new Date().toLocaleTimeString()}`);
                                } catch (error) {
                        ...
                        }, 10 * 60 * 1000); // Every 10 minutes (safe)
                        ...
                        function setupAutoReconnect() {
                                ...
                                autoReconnectInterval = setInterval(async () => {
                                        try {
                                                // Check if API is still responsive (use getAppState, not getUserInfo to avoid FB block)
                                                const state = api.getAppState();
                                                if (!state || state.length === 0) throw new Error("No appstate");
                        ...
                        // Load handlerAction once (not on every event)
                        let handlerAction;
                        try {
                                handlerAction = require("../handler/handlerAction.js")(
                                        api,
                                        threadModel,
                                        userModel,
                                        dashBoardModel,
                                ...
                                log.err("HANDLER", "handlerAction.js load failed: " + handlerErr.message);
                        }
                        ...
                        async function callBackListen(error, event) {
                                ...
                                if (false && configLog.disableAll === false && configLog[event.type] !== false) {
