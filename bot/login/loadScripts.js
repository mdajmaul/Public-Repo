...
// Safe fallback for when global.utils.colors isn't loaded yet.
// colors.hex("#fff") returns a function that takes text -> text (nested call pattern).
// colors.red(text) / colors.green(text) / colors.yellow(text) etc return text directly.
const identityColors = new Proxy({}, {
        get: (target, prop) => {
                if (prop === 'hex') return () => (t) => t;
                return (t) => t;
        }
});
const colors = _colors || identityColors;
