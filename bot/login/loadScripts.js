...
// Safe fallback: works for colors.red(text), colors.green(text), colors.hex("#fff")(text), etc.
// even if global.utils.colors hasn't loaded yet or is missing some method.
const identityColors = new Proxy({}, {
        get: () => (...args) => args.length === 1 && typeof args[0] === 'string' ? args[0] : ((t) => t)
});
