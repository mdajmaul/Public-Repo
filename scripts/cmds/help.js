...
module.exports = {
  ...
  onStart: async function ({ message, args, prefix }) {
    ...
    try {
      ...
      gifReady = fs.existsSync(gifPath) && fs.statSync(gifPath).size > 0;
    } catch (e) {
      gifReady = false;
    }
    ...
    if (args[0]) {
      ...
      if (gifReady) {
        return message.reply({ body: infoBody, attachment: fs.createReadStream(gifPath) });
      } else {
    ...
    if (gifReady) {
      return message.reply({ body: msg, attachment: fs.createReadStream(gifPath) });
    } else {
